package SWFStatsTest
{
	...
	public final class GameVars
	{
		...
		{
			Log.Output("SWFStatsTest.GameVars.Load");

			if(Callback != null)
			{
				Log.Output("** ERROR LOADING GAMEVARS **\n" + 
					   "    A previous command to load the GameVars is still in progress.");
				return;
			}
			...
			loader.load(new URLRequest("http://utils.SWFStatsTest.com/gamevars/load.aspx?swfid=" + Log.SWFID + "&guid=" + Log.GUID + "&" + Random));
		}

		private static function ProcessData(e:Event):void
		{
			Log.Output("SWFStatsTest.GameVars.ProcessData");	
		...
		{
			og.Output("** ERROR COMPLETING REQUEST ~ SWFStatsTest.GameVars **");
			Callback(null);
			Callback = null;
		}
